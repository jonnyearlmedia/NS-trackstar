...
const PROJECT_SOURCE = "trackstar-v2-shapes";
const POINT_SOURCE = "trackstar-v2-points";
const SELECTED_SOURCE = "trackstar-v2-selected";
...
export function MapCanvasV2({
  ...
  useEffect(() => {
    ...
    async function mount() {
      ...
      // Keep the native control functional for the explicit Near Me trigger, but hide its button.
      ...
      async function refresh() {
        ...
        try {
          ...
          const ids = new Set(categoryProjects.map((project) => project.id));
          const categoryFeatures = allConsumerFeatures.filter((feature) => ids.has(String(feature.properties?.id ?? "")));

          const floor = categoryRef.current === "all" ? minimumPriorityForZoom(map.getZoom()) : 0;
          const displayFeatures = categoryFeatures.filter((feature) => Number(feature.properties?.display_priority ?? 0) >= floor);
          ...
          onCoverageRef.current?.({
            visibleMapped: categoryProjects.length,
            mappedMatching: Number(metadata?.mapped_matching ?? categoryProjects.length),
            locationPending: Number(metadata?.location_pending ?? 0),
            totalMatching: Number(metadata?.total_matching ?? categoryProjects.length),
          });
      ...
      map.on("load", () => {
        ...
        map.addLayer({ id: "v2-selected-fill", type: "fill", source: SELECTED_SOURCE, filter: ["in", ["geometry-type"], ["literal", ["Polygon", "MultiPolygon"]]], paint: { "fill-color": "#d9ff61", "fill-opacity": 0.34 } });
        map.addLayer({ id: "v2-selected-line", type: "line", source: SELECTED_SOURCE, paint: { "line-color": "#f5ffc9", "line-width": 3.5, "line-opacity": 1 } });
        map.addLayer({ id: "v2-selected-point", type: "circle", source: SELECTED_SOURCE, filter: ["==", ["geometry-type"], "Point"], paint: { "circle-radius": 9, "circle-color": "#d9ff61", "circle-stroke-color": "#f7ffd7", "circle-stroke-width": 3 } });

        for (const layer of ["v2-fill", "v2-fill-uncertain", "v2-line", "v2-line-uncertain", "v2-points", "v2-points-uncertain"]) {
        ...
        map.on("mouseenter", "v2-clusters", () => { map.getCanvas().style.cursor = "pointer"; });
        map.on("mouseleave", "v2-clusters", () => { map.getCanvas().style.cursor = ""; });
        map.on("click", "v2-clusters", async (event) => {
