...
export function installFinalLayers(
  ...
  map.addLayer({
    ...
    type: "fill",
    source: SOURCES.mask,
    paint: { "fill-color": "#020705", "fill-opacity": 0.64 },
  });
  map.addLayer({
    id: "final-edge",
    type: "line",
    source: SOURCES.edge,
    paint: {
      "line-color": "rgba(217,255,97,0.44)",
      "line-width": 1.2,
      "line-dasharray": [3, 3],
    },
  });
  ...
  map.addLayer({
    ...
    paint: {
      ...
      "fill-opacity": ["interpolate", ["linear"], ["zoom"], 7, 0.05, 11, 0.16, 15, 0.28],
    },
  });
  map.addLayer({
    id: "final-fill-u",
  ...
  map.addLayer({
    ...
    paint: {
      ...
      "line-width": ["interpolate", ["linear"], ["zoom"], 7, 1, 13, 2.4, 17, 4],
      "line-opacity": 0.75,
    },
  });
  map.addLayer({
  ...
  map.addLayer({
    ...
    paint: {
      "circle-radius": ["interpolate", ["linear"], ["zoom"], 7, 4.5, 12, 6.2, 15, 8],
  ...
  map.on("click", (event) => {
    ...
    if (projects.length > 1) onMany(projects);
    else if (projects[0]) onOne(projects[0]);
  });
...
export function updateSelected(map: MapLibreMap, project: MapProject | null) {
  ...
  source.setData({
    ...
    properties: { id: project.id, name: project.name, consumer_category: project.consumerCategory },
  });
}
