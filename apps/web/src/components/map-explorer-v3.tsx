...
export function MapExplorerV3({ initialProjectId }: { initialProjectId?: string }) {
  ...
  return (
    <section className={`trackstarApp ${styles.root}`} aria-label="Trackstar Napa Solano project map">
      ...
      <header className={styles.topBar}>
        ...
        <button className={styles.searchButton} onClick={() => setSearchOpen(true)} type="button"><SearchIcon /><span>Search address, road or project</span></button>
      </header>

      {view === "explore" && !selected && !browseOpen ? (
      ...
      {view === "explore" && !selected && !browseOpen && exploreMode === "orientation" ? (
        <section className={styles.orientationPanel} aria-label="Browse this area">
          ...
          <div className={styles.areaHeading}>
            <div><p>EXPLORE</p><h2>{areaLabel}</h2><span>{viewport ? `${visibleCount.toLocaleString()} mapped projects in view` : "Loading project coverage…"}</span></div>
          ...
          <div className={styles.categoryGrid}>
            {CATEGORIES.filter((item) => item.key !== "all").map((item) => (
              <button className={category === item.key ? styles.categorySelected : ""} key={item.key} onClick={() => applyCategory(item.key)} type="button">
                <i>{item.icon}</i><span>{item.label}</span>
              </button>
            ))}
          </div>
          <div className={styles.panelFooter}>
            <button onClick={() => { rememberFreeExplore(); setBrowseOpen(true); }} type="button">Browse projects</button>
            <button onClick={() => setFilterOpen(true)} type="button"><FilterIcon /> More filters</button>
          </div>
        </section>
      ) : null}

      {view === "explore" && !selected && !browseOpen && exploreMode === "free" ? (
        <section className={styles.freeExploreBar} aria-label="Map categories">
          <button className={styles.areaPill} onClick={() => setExploreMode("orientation")} type="button">
            <span><strong>{areaLabel}</strong><small>{viewport ? `${visibleCount.toLocaleString()} showing` : "Loading"}</small></span><ChevronIcon up />
          </button>
      ...
      {view === "explore" && !selected && browseOpen ? (
        <section className={styles.updatesPanel} aria-label="Projects in this area">
          <div className={styles.updatesHeader}>
            <div><p>BROWSE THIS AREA</p><h2>{currentCategory.longLabel}</h2><span>{visibleCount.toLocaleString()} mapped projects in the current view</span></div>
            <div className={styles.projectActions}><button aria-label="Close project list" onClick={() => setBrowseOpen(false)} type="button">×</button></div>
          </div>
          {category === "all" && visibleCount > 150 ? (
            <>
              <p className={styles.stateMessage}>There are too many projects here for a useful flat list. Choose a category or zoom in.</p>
              <div className={styles.categoryGrid}>
                {CATEGORIES.filter((item) => item.key !== "all").map((item) => (
                  <button key={item.key} onClick={() => applyCategory(item.key)} type="button"><i>{item.icon}</i><span>{item.label}</span></button>
                ))}
              </div>
            </>
          ) : (
            <ol className={styles.updateList}>
              {sortedViewportProjects.map((project) => (
                <li key={project.id}><button onClick={() => selectProject(project)} type="button"><small>{consumerType(project.projectType, project.name)}{project.deliveryStage ? ` · ${readableValue(project.deliveryStage)}` : ""}</small><strong>{project.name}</strong><span>{project.locationUncertain ? "Approximate location" : "View project"}</span></button></li>
              ))}
            </ol>
          )}
      ...
      <nav className={styles.bottomNav} aria-label="Trackstar sections">
        ...
        <button className={view === "updates" ? styles.navSelected : ""} onClick={() => { clearSelection(); setBrowseOpen(false); setView("updates"); }} type="button">Updates</button>
      </nav>
      ...
      {view === "explore" && selected ? (
        <article className={`${styles.projectCard} ${detailExpanded ? styles.projectCardExpanded : ""}`}>
          ...
          <div className={styles.projectTopLine}>
            <button className={styles.breadcrumb} onClick={clearSelection} type="button">‹ {consumerType(selected.projectType, selected.name)}</button>
            <div className={styles.projectActions}><button aria-label="Share project" onClick={() => void shareProject()} type="button"><ShareIcon /></button><button aria-label="Close project" onClick={clearSelection} type="button">×</button></div>
          </div>
          <div className={styles.projectScroll}>
            ...
            {detailExpanded ? <div className={styles.detailLayer}>
      ...
      {searchOpen ? <aside className={styles.searchOverlay} aria-label="Search Trackstar">
        ...
        <div className={styles.searchBody}>
          ...
          <div className={styles.searchResults}>{searchResults.map((result) => <button key={result.id} onClick={() => chooseSearchResult(result)} type="button"><span><small>{consumerType(result.project_type, result.name)}</small><strong>{result.name}</strong>{result.summary ? <em>{result.summary}</em> : null}</span><b>›</b></button>)}</div>
        </div>
      </aside> : null}
    </section>
  );
