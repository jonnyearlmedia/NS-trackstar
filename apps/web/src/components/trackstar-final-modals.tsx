...
import { CategoryIcon } from "./trackstar-final-icons";
...
export function FilterSheet({ category, lifecycle, timeWindow, viewport, onCategory, onLifecycle, onTime, onClear, onDone, onClose }: {
  ...
  return <aside className={styles.scrim} onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <section className={styles.modalSheet} aria-label="Filter Trackstar">
      ...
      <section><h3>Category</h3><div className={styles.filterChoices}>{CATEGORIES.map((item) => <button data-category={item.key} className={category === item.key ? styles.filterChoiceSelected : ""} key={item.key} onClick={() => onCategory(item.key)} type="button"><span className="filterChoiceLabel"><span className="filterChoiceIcon" data-category={item.key}><CategoryIcon category={item.key} /></span><span>{item.longLabel}</span></span><i>{category === item.key ? "✓" : ""}</i></button>)}</div></section>
...
export function OverlapChooser({ projects, onClose, onSelect }: { projects: MapProject[]; onClose: () => void; onSelect: (project: MapProject) => void }) {
  return <aside className={styles.scrim} onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><section className={styles.modalSheet}><header><div><p>MULTIPLE PROJECTS</p><h2>What's here?</h2></div><button aria-label="Close chooser" onClick={onClose} type="button">×</button></header><div className={extra.overlapList}>{projects.map((project) => <button key={project.id} onClick={() => onSelect(project)} type="button"><span className="overlapCategoryIcon" data-category={project.consumerCategory}><CategoryIcon category={project.consumerCategory} /></span><span><small>{categoryLabel(project.consumerCategory)} · {lifecycleLabel(project.lifecycleStage)}</small><strong>{project.name}</strong></span></button>)}</div></section></aside>;
}
