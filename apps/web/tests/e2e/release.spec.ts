...
test("dense regional inventory remains browsable instead of becoming a GIS wall", async ({ page }, testInfo) => {
  test.skip(!testInfo.project.name.includes("chromium"), "density gate runs once");
  await openTrackstar(page);
  await waitForProjectCoverage(page);

  const browse = page.getByRole("button", { name: /Browse [\d,]+ projects/i });
  const label = await browse.innerText();
  expect(Number(label.replace(/[^0-9]/g, ""))).toBeGreaterThan(100);
...
test("classified search opens a real record", async ({ page }, testInfo) => {
  test.skip(!testInfo.project.name.includes("chromium"), "search gate runs once");
  await openTrackstar(page);
...
test("slow backend responses keep the shell understandable and recover cleanly", async ({ page }, testInfo) => {
  test.skip(!testInfo.project.name.includes("chromium"), "network gate runs once");
...
test("initial surface has no serious or critical accessibility violations", async ({ page }, testInfo) => {
  test.skip(!testInfo.project.name.includes("chromium"), "axe gate runs once");
  await openTrackstar(page);
  await waitForProjectCoverage(page);
