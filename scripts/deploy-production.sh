...
case "$mode" in
  ...
  deploy)
    ;;
  *)
    echo "Usage: ./scripts/deploy-production.sh [deploy|configure|status]" >&2
    exit 2
    ;;
esac
...
if [ ! -f "$env_file" ]; then
  ...
  echo "Using saved deployment settings. Run './scripts/deploy-production.sh configure' to change them without editing files."
fi
